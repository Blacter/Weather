...
class OpenWeatherWorks:
    def __init__(self) -> None:
        ...
        self.weather_by_lat_and_lon_url_template: Template = OPEN_WEATHER_PRINTS[
            'weather_by_lat_and_lon_url']
        self.weather_by_city_and_country_template: Template = OPEN_WEATHER_PRINTS[
            'weather_by_city_and_country']
    ...
    def set_url_with_lat_and_lon(self) -> None:
        self.url_with_lat_and_lon: str = self.weather_by_lat_and_lon_url_template.substitute(
    ...
    def set_url_with_city_and_country(self) -> None:
        self.url_with_city_and_country: str = self.weather_by_city_and_country_template.substitute(
            city_name=self.city_name, country_code=self.country_code)

    def get_weather_response_by_city_and_country(self) -> None:
        self.weather_response: requests.Response | None
        try:
            self.weather_response = requests.get(
                self.url_with_city_and_country)
        except RequestException:
            self.weather_response = None
    ...
    def handle_temperature(self) -> str | None:
        location_temperature: str | None = self.weather.get(
            'main', {}).get('temp')
